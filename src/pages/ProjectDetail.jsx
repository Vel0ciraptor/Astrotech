  ...
  return (
    <>
      <div className="pd-carousel-wrap" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
        ...
        {images.length > 1 && (
          <>
            <button className="pd-carousel-arrow pd-carousel-arrow-prev" onClick={prev} disabled={current === 0}>
              <ChevronLeft size={20} />
            </button>
            <button className="pd-carousel-arrow pd-carousel-arrow-next" onClick={next} disabled={current === images.length - 1}>
        ...
        {images.length > 1 && (
          <div className="pd-carousel-dots">
            {images.map((_, i) => (
              <button key={i} className={`pd-carousel-dot ${i === current ? 'active' : ''}`} onClick={() => setCurrent(i)} />
            ))}
          </div>
        )}
      </div>
  ...
  return (
    <div className="pd-page">
      ...
      <section className="pd-carousel-section">
        <div className="container">
          <ImageCarousel images={project.images} color={c} />
        </div>
      </section>
      ...
      <section className="pd-content">
        <div className="container">
          <div className="pd-content-grid">
            <div>
              <div className="pd-section-tag" style={{ background: `${c}15`, color: c }}>
                El reto
              </div>
              <p className="pd-section-body">
                {project.challenge || 'Desarrollar una solución escalable y eficiente que mejore los procesos actuales y proporcione una ventaja competitiva real al cliente.'}
              </p>
              ...
              <p className="pd-section-body">
                {project.solution || 'Implementación de una arquitectura moderna utilizando las mejores prácticas de desarrollo, con enfoque en rendimiento, usabilidad y escalabilidad.'}
              </p>
            </div>

            <div className="pd-sidebar">
              <div className="pd-sidebar-card" style={{ borderColor: `${c}30` }}>
                <div className="pd-sidebar-label">Resultado alcanzado</div>
                <div className="pd-result-big">
                  <CheckCircle2 size={20} style={{ color: c, flexShrink: 0, marginTop: 4 }} />
                  <div className="pd-result-big-text">{project.result}</div>
                </div>
              </div>

              {project.stack && project.stack.length > 0 && (
                <div className="pd-sidebar-card">
                  <div className="pd-sidebar-label">Stack tecnológico</div>
              ...
              <div className="pd-sidebar-card">
                <div className="pd-sidebar-label">Área del proyecto</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontWeight: 700, color: c }}>
                  {getCategoryIcon(project.category)}
                  {getCategoryName(project.category)}
                </div>
              </div>
